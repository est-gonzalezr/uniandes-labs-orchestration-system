...
case class DownloadingConsumer(
  ...
  override def handleDelivery(
    ...
    val ftpDownloadingPath = messageMap("ftp_downloading_path")

    val operationResult =
      FtpWorker.fileFromPath(ftpWorker, ftpDownloadingPath) match
        case Left(error) =>
          handleError(error, messageMap, channel, failKey)
          Left(error)
        case Right(fileBytes) =>
          FileManager.fileToLocal(fileBytes, ftpDownloadingPath) match
            case Left(error) =>
              handleError(error, messageMap, channel, failKey)
              Left(error)
            case Right(_) =>
              Right(messageMap + ("local_file_path" -> ftpDownloadingPath))
    ...
    handleFtpDelete(ftpDownloadingPath)
